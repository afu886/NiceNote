...
- `rollout.max_fix_attempts`: Auto-fix budget for each batch. When Claude Code exits non-zero or a verification command fails, the runner feeds the error output back into a follow-up Claude Code invocation up to this many times before declaring the batch `failed`. Defaults to `3`. Set to `0` to fail fast on the first error.
