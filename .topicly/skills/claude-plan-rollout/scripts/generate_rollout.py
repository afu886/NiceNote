...
#                      [--force] [--dry-run] [--commit-per-batch] [--claude-cmd CMD] [--model MODEL]
...
#   --claude-cmd CMD         覆盖默认的 Claude Code CLI 命令模板（使用 {repo} 占位符）。
#   --model MODEL            覆盖 rollout 计划里的模型配置（追加 --model <id>）。
#   --reset-batch BATCH_ID   将指定 batch 的状态重置为 pending。
#   --max-fix-attempts N     覆盖计划里的最大自动修复轮次（默认 3）。每次失败后 runner 会把
#                            错误信息（claude CLI 输出或 verify 命令输出）作为修复上下文重新交给
#                            Claude Code，由 Claude 诊断并修复，然后再次跑 verify；只有耗尽该
#                            轮次仍未恢复时才把 batch 标记为 failed 并退出。
...
_BASH_EXECUTABLE = shutil.which("bash") if os.name == "nt" else None
...
def build_claude_retry_notes(batch: Batch, claude_failure: ClaudeFailure, retry_number: int) -> str:
    return "\n".join(
        [
            ...
            "",
            f"Your previous invocation on batch `{batch.id}` exited with a non-zero status before",
            "the batch could be declared done. The runner is automatically handing the failure",
            "back to you so you can diagnose and repair the problem before it gives up.",
            "",
            f"- Fix attempt: {retry_number} of the configured maximum",
            "- Working tree: any partial changes from the last attempt are still on disk; build on them",
            "  rather than starting over.",
            "",
            "### What to do now",
            "1. Read the error output below and identify the root cause (tool error, syntax error,",
            "   missing dependency, command not available, etc.).",
            "2. Make the smallest set of edits that resolves the failure without expanding scope.",
            "3. Re-run the verification commands listed above yourself. Do not declare success until",
            "   every command exits 0.",
            "4. If the failure is environmental (missing tool, network, etc.) and cannot be fixed",
            "   from inside the repo, say so explicitly so the human can intervene.",
            "",
            "### Previous Claude Code CLI Failure",
...
def build_verify_retry_notes(batch: Batch, verify_result: VerifyResult, retry_number: int) -> str:
    parts = [
        "## You Are In Auto-Fix Mode",
        "",
        f"Your previous work on batch `{batch.id}` finished, but the runner's verification",
        "commands failed. The runner is automatically handing the failure back to you so the",
        "batch can be repaired and re-verified before it gives up.",
        "",
        f"- Fix attempt: {retry_number} of the configured maximum",
        "- Working tree: your previous edits are still in place; refine them instead of redoing",
        "  the whole batch.",
        "",
        "### What to do now",
        "1. Read each failed command's output below and identify the underlying defect (logic",
        "   bug, type error, lint violation, missing artifact, flaky test, etc.).",
        "2. Make the minimum changes required to fix every failing check without breaking checks",
        "   that already passed.",
        "3. Re-run each failing verification command yourself. Only stop once they all exit 0.",
        "4. If a check is genuinely broken (wrong assertion, stale snapshot, etc.) and not a code",
        "   defect, say so explicitly with evidence so the human can intervene.",
        "",
    ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--claude-cmd", dest="claude_cmd", help="Override the Claude Code command template")
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Auto-fix budget after Claude Code or verification failures. Each failure feeds "
            "the error output back into Claude Code as a fix prompt; the runner only gives up "
            "once this budget is exhausted. Defaults to the plan value (3 if unset)."
        ),
    )
...
def main() -> int:
    ...
    max_fix_attempts = (
        ROLLOUT.get("max_fix_attempts", 3)
    ...
    for batch_id in selected_batch_ids:
        ...
        while True:
            in_fix_mode = attempt > 0
            if in_fix_mode:
                append_log(
                    log_path,
                    f"\n# auto-fix attempt {attempt} for {batch.id}\n",
                )
                print(
                    c(
                        f"⚒ {batch.id} auto-fix attempt {attempt}/{max_fix_attempts} — "
                        f"handing error context back to claude",
                        Colors.YELLOW,
                    )
                )
            ...
            if rc != 0:
                ...
                if attempt < max_fix_attempts:
                    ...
                    print(
                        c(
                            f"↺ {batch.id} claude exited with {rc}; queuing auto-fix "
                ...
                print(
                    c(
                        f"✗ {batch.id} claude exited with {rc} after {fix_attempts} auto-fix "
                        f"attempt(s) ({elapsed:.0f}s)",
                        Colors.RED,
                    )
                )
                return rc
            ...
            if verify_result.ok:
                ...
                if fix_attempts:
                    print(
                        c(
                            f"✔ {batch.id} complete after {fix_attempts} auto-fix attempt(s) "
                            f"({elapsed:.0f}s)",
                            Colors.GREEN,
                        )
                    )
            ...
            if attempt >= max_fix_attempts:
                ...
                print(
                    c(
                        f"✗ {batch.id} failed verification after {fix_attempts} auto-fix "
                        f"attempt(s)",
                        Colors.RED,
                    )
                )
            ...
            print(
                c(
                    f"↺ {batch.id} verification failed; queuing auto-fix "
                    f"({attempt}/{max_fix_attempts})",
                    Colors.YELLOW,
                )
            )
...
def validate_rollout(raw: dict, plan_path: Path) -> dict:
    ...
    rollout = {
        ...
        "model": raw.get("model"),
        "max_fix_attempts": as_int(raw.get("max_fix_attempts", 3), "rollout.max_fix_attempts"),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate rollout.py (Claude Code) from a Markdown implementation plan.")
...
def main() -> int:
    ...
    print(f"[OK] Generated Claude Code rollout runner: {output_path}")
